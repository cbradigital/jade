...
function initWithData() {
    ...
    for (var s in jade.sectionFields) {
        ...
        // Numbers
        var p = jQuery("#" + section).closest(".elementor-container").parent().parent();
        ...
        for (var a in sectionData.analysis) {
            ...
            rowHTML = rowHTML.replace("{{result-alternatives}}", row.alternative.stripSlashes());
            ...
            } else if (words.findIndex(e => e.highlight == row.word && e.className != colorClass) >= 0) {
                words.push({
                    highlight: row.word,
...
jade.printPDF = () => {
    ...
    if (renderDivHeader == false) {
        jQuery("body").append('<div id="renderdivheader" style="zoom: ' + zoom + '%;"></div>');
        jQuery("body").append('<div id="renderdivfooter" style="zoom: ' + zoom + '%;"></div>');
    ...
    topPart.find("[left-and-svg-cell]").removeClass("elementor-col-50").addClass("elementor-col-100");
    topPart.appendTo(renderDivHeader);

    var filename = 'JobAdDecoder_' + jade.title.trim().replace(/[^a-z0-9]/gi, '_') + '.pdf';
    ...
    for (let j = 0; j < pages.length - 1; j++) {
        ...
        part.find("[kill-this-pdf]").prev().removeClass("elementor-col-50").addClass("elementor-col-100");
        part.find("[kill-this-pdf]").remove();

        worker = worker.get('pdf').then(
            pdf => { pdf.addPage() }
        ).from(part[0]).toContainer().toCanvas().toPdf();
    ...
    worker = worker.get('pdf').then(pdf => { pdf.addPage() }).from(renderDivFooter[0]).toContainer().toCanvas().toPdf();
...
jade.replaceClassForPDF = (element, remove) => {
    if (remove) {
        element.find(".hwt-container-pdf").addClass("hwt-container").removeClass("hwt-content-pdf");
        element.find(".hwt-backdrop-pdf").addClass("hwt-backdrop").removeClass("hwt-backdrop-pdf");
        element.find(".hwt-highlights-pdf").addClass("hwt-highlights").removeClass("hwt-highlights-pdf");
        element.find(".hwt-input-pdf").addClass("hwt-input").removeClass("hwt-input-pdf");
        element.find(".hwt-content-pdf").addClass("hwt-content").removeClass("hwt-content-pdf");
        //element.find(".divCellSVG-pdf").addClass("divCellSVG").removeClass("divCellSVG-pdf");

    } else {

        element.find(".hwt-container").addClass("hwt-container-pdf").removeClass("hwt-container");
        element.find(".hwt-backdrop").addClass("hwt-backdrop-pdf").removeClass("hwt-backdrop");
        element.find(".hwt-highlights").addClass("hwt-highlights-pdf").removeClass("hwt-highlights");
        element.find(".hwt-input").addClass("hwt-input-pdf").removeClass("hwt-input");
        element.find(".hwt-content").addClass("hwt-content-pdf").removeClass("hwt-content");
...
jade.printPopup = (show) => {
    ...
    if (!jade.modal) {
        jQuery("body").append("<div id=\"print-modal\">\n" +
            "\n" +
            "  <!-- Modal content -->\n" +
